    ...
    public function updateUser($post)
    {
        ...
        Pengguna baru berhasil diubah!!!</div>');
            ...
            if ($result->num_rows() < 1) {
                ...
                $alamat = [
                    'alamat' => $leng . " , desa " . $de . ", kec " . kecamatan($ke) . ", kota/kab " . kota($ko) . ", provinsi " . provinsi($pr) . "( " . $patokan . " )"
                ];
                $this->db->update('user', $alamat, ['id' => $id]);
            } else {
                $data = [
                    'lengkap' => $leng,
                    'provinsi' => $pr,
                ...
                $this->db->update('alamat', $data, ['id_user' => $id]);
                $alamat = [
                    'alamat' => $leng . " , desa " . $de . ", kec " . kecamatan($ke) . ", kota/kab " . kota($ko) . ", provinsi " . provinsi($pr) . "( " . $patokan . " )"
                ];
                $this->db->update('user', $alamat, ['id' => $id]);
